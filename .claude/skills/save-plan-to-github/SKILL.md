...
- **`--repo` is mandatory** on every `gh` call here: all three repos are forks of **archived** CartoDB originals, so without it `gh` targets upstream and fails read-only. Issues live in `massif-maps/MassifMaps`; submodule-only issues in `massif-maps/massif-maps-libs` or `massif-maps/massif-external-libs`. When the work spans repos, the issue comment goes on the issue's own repo — and names the other repos it will touch.
