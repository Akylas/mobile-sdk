...
#include "MassifGeocodingProxy.h"
...
namespace {
    ...
    struct ValueConverter {
        massif::Variant operator() (std::monostate) const { return massif::Variant(); }
        template <typename T> massif::Variant operator() (T val) const { return massif::Variant(val); }
    };
...
namespace massif {
    ...
    std::vector<std::shared_ptr<GeocodingResult> > MassifGeocodingProxy::CalculateAddresses(const std::shared_ptr<geocoding::Geocoder>& geocoder, const std::shared_ptr<GeocodingRequest>& request) {
    ...
    std::vector<std::shared_ptr<GeocodingResult> > MassifGeocodingProxy::CalculateAddresses(const std::shared_ptr<geocoding::RevGeocoder>& revGeocoder, const std::shared_ptr<ReverseGeocodingRequest>& request) {
    ...
    MassifGeocodingProxy::MassifGeocodingProxy() {
    }

    std::shared_ptr<GeocodingResult> MassifGeocodingProxy::TranslateAddress(const std::shared_ptr<Projection>& proj, const geocoding::Address& addr, float rank) {
        std::vector<std::shared_ptr<Feature> > features;
        std::transform(addr.features.begin(), addr.features.end(), std::back_inserter(features), std::bind(&MassifGeocodingProxy::TranslateFeature, proj, std::placeholders::_1));
    ...
    std::shared_ptr<Feature> MassifGeocodingProxy::TranslateFeature(const std::shared_ptr<Projection>& proj, const geocoding::Feature& feature) {
    ...
    std::shared_ptr<Geometry> MassifGeocodingProxy::TranslateGeometry(const std::shared_ptr<Projection>& proj, const std::shared_ptr<geocoding::Geometry>& geom) {
        ...
        if (auto pointGeom = std::dynamic_pointer_cast<geocoding::PointGeometry>(geom)) {
            ...
            std::transform(multiGeom->getGeometries().begin(), multiGeom->getGeometries().end(), std::back_inserter(geometries), std::bind(&MassifGeocodingProxy::TranslateGeometry, proj, std::placeholders::_1));
