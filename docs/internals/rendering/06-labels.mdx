...
- The switch is `debug.massif.labelocclusion` for now; the style/option property that should drive
  it is not written. mapbox makes it a paint property per layer with the opacity an occluded symbol
  keeps, which is the shape to mirror.
