...
@interface DemoPeaksListener : NTVectorTileEventListener
@end

@implementation DemoPeaksListener

- (BOOL)onVectorTileClicked:(NTVectorTileClickInfo *)clickInfo {
    NTVariant *properties = [[clickInfo getFeature] getProperties];
...
@implementation DemoMap {
    NSMutableDictionary<NSNumber *, NTLayer *> *_layers;

    // --- map objects the panel reaches through the properties ---
    NTHillshadeRasterTileLayer *_hillshadeLayer;   // stand-alone hillshade layer, when built
    NTVectorTileLayer *_baseLayer;                 // base map layer, whatever the mode
    NTCompositeVectorTileLayer *_compositeLayer;   // same object as _baseLayer in COMPOSITE mode
    NTMBVectorTileDecoder *_baseDecoder;
    NTContourTileDataSource *_contourSource;       // shared by the layer and the composite slot
    NTPostProcessEffect *_reliefEffect;

    // --- shared, lazily created tile sources ---
    NTTileDataSource *_cachedDem;
    NTTileDataSource *_cachedVector;
    NTTileDataSource *_cachedRaster;
    NTTileDataSource *_cachedContourTiles;
    NTGeoJSONVectorTileDataSource *_cachedManeuvers;
    int _maneuverLayerIndex;
    NTManeuverArrowBuilder *_maneuverBuilder;
    NSMutableDictionary<NSNumber *, NTFeatureCollection *> *_maneuverArrows;
    ...
    NTColor *_savedClearColor;
    NTColor *_savedSkyColor;
    NTBitmap *_savedBackgroundBitmap;
    BOOL _appearanceSaved;

    BOOL _nutiParameterOn;
}
...
- (instancetype)initWithMapView:(NTMapView *)mapView {
    if ((self = [super init])) {
        ...
        _maneuverBuilder = [[NTManeuverArrowBuilder alloc] init];
        _maneuverLayerIndex = -1;
        _savedViewDistance = 1;
        _nutiParameterOn = YES;
        _compositeStatus = @"";
...
- (NTLayer *)createLayer:(DemoFeature)feature {
    switch (feature) {
        ...
        case DemoFeatureSatellite:    return [[NTRasterTileLayer alloc] initWithDataSource:[self rasterSource]];
...
- (NTLayer *)createBaseLayer {
    ...
    if (![[DemoConfig stringFor:@"base"] isEqualToString:@"composite"]) {
        _compositeLayer = nil;
        _baseLayer = [[NTVectorTileLayer alloc] initWithDataSource:[self vectorSource] decoder:_baseDecoder];
        [_baseLayer setTileCacheCapacity:cacheBytes];
        return _baseLayer;
    }

    NTCompositeVectorTileLayer *layer =
        [[NTCompositeVectorTileLayer alloc] initWithDataSource:[self vectorSource] decoder:_baseDecoder];
    ...
    if ([[DemoConfig stringFor:@"style"] isEqualToString:@"nuti"]
            && [DemoConfig floatFor:@"nutiInterval"] > 0) {
        [self startNutiToggleLoop];
    }
    return layer;
}
...
- (void)checkCompositeSlots {
    ...
    NTStringVector *styleLayers = [_baseDecoder getStyleLayerNames];
    ...
    NTStringVector *registered = [_compositeLayer getExternalDataSourceNames];
    ...
    NSLog(@"CartoDemo: %@ | style layers: %@", _compositeStatus, declared);
    if (missing) {
        NSLog(@"CartoDemo: a slot is missing: the style declares no layer with that name. Use a "
...
- (void)applyHillshadeConfig {
    NTHillshadeRasterTileLayer *layer = _hillshadeLayer;
    if (!layer) {
        return;
    }
    ...
    [layer setIlluminationDirection:[[NTMapVec alloc] initWithX:sin(radians)
                                                              y:cos(radians)
...
- (NTLayer *)createHypsoLayer {
    NTCustomRasterTileLayer *layer = [[NTCustomRasterTileLayer alloc] initWithDataSource:[self demSource]];
...
- (NTLayer *)createContourLayer {
    ...
    return [[NTVectorTileLayer alloc] initWithDataSource:[self contourSource] decoder:decoder];
}
 ...
 */
- (NTLayer *)createContourTilesLayer {
    NTMBVectorTileDecoder *decoder = [[NTMBVectorTileDecoder alloc] initWithCartoCSSStyleSet:
        [[NTCartoCSSStyleSet alloc] initWithCartoCSS:[DemoStyles contourTilesStyle]]];
    return [[NTVectorTileLayer alloc] initWithDataSource:[self contourTilesSource] decoder:decoder];
}
 ...
 */
- (NTLayer *)createPeaksLayer {
    NTMBVectorTileDecoder *decoder = [[NTMBVectorTileDecoder alloc] initWithCartoCSSStyleSet:
        [[NTCartoCSSStyleSet alloc] initWithCartoCSS:[DemoStyles peaksStyle]]];
    NTVectorTileLayer *layer = [[NTVectorTileLayer alloc] initWithDataSource:[self vectorSource]
                                                                     decoder:decoder];
    [layer setLabelRenderOrder:NT_VECTOR_TILE_RENDER_ORDER_LAST];
...
- (NTLayer *)createElementsLayer {
    ...
    [markerStyle setColor:[[NTColor alloc] initWithR:255 g:0 b:0 a:255]];
    ...
    for (int i = 0; i < 4; i++) {
        NTMapPos *pos = [projection fromWgs84:[[NTMapPos alloc] initWithX:peaks[i][0] y:peaks[i][1]]];
        [source add:[[NTMarker alloc] initWithPos:pos style:[markerStyle buildStyle]]];
    }

    NTLineStyleBuilder *lineStyle = [[NTLineStyleBuilder alloc] init];
    [lineStyle setWidth:8];
    [lineStyle setColor:[[NTColor alloc] initWithR:0 g:90 b:255 a:255]];
    NTMapPosVector *linePoses = [[NTMapPosVector alloc] init];
    [linePoses add:[projection fromWgs84:[[NTMapPos alloc] initWithX:5.6800 y:45.1600]]];
    [linePoses add:[projection fromWgs84:[[NTMapPos alloc] initWithX:5.7247 y:45.1927]]];
    [linePoses add:[projection fromWgs84:[[NTMapPos alloc] initWithX:5.7869 y:45.2876]]];
    [source add:[[NTLine alloc] initWithPoses:linePoses style:[lineStyle buildStyle]]];
 ...
 */
- (NTLayer *)createManeuversLayer {
    NTMBVectorTileDecoder *decoder = [[NTMBVectorTileDecoder alloc] initWithCartoCSSStyleSet:
        [[NTCartoCSSStyleSet alloc] initWithCartoCSS:[DemoStyles maneuverStyle:[self maneuverHeadPath]]]];
    NTGeoJSONVectorTileDataSource *source = [self maneuverSource];
    if (!_seededManeuvers) {
...
- (NTGeoJSONVectorTileDataSource *)maneuverSource {
    if (!_cachedManeuvers) {
        _cachedManeuvers = [[NTGeoJSONVectorTileDataSource alloc] initWithMinZoom:0 maxZoom:24];
...
- (void)rebuildManeuverLayer {
    ...
    NTFeatureVector *features = [[NTFeatureVector alloc] init];
    for (NTFeatureCollection *arrow in _maneuverArrows.allValues) {
    ...
    [_cachedManeuvers setLayerFeatureCollection:_maneuverLayerIndex
                                     projection:nil
                              featureCollection:[[NTFeatureCollection alloc] initWithFeatures:features]];
}
...
- (NSString *)maneuverHeadPath {
    ...
    if (!svg) {
        NSLog(@"CartoDemo: maneuver svg not readable: %@", name);
        return @"";
    }
    ...
    if (!match) {
        NSLog(@"CartoDemo: maneuver svg carries no path: %@", name);
        return @"";
    }
...
- (NTContourTileDataSource *)contourSource {
    if (!_contourSource) {
        _contourSource = [[NTContourTileDataSource alloc] initWithDataSource:[self demSource]
                                                            elevationDecoder:[self elevationDecoder]];
...
- (void)applyTerrainOptions {
    if (!_terrainOptions) {
        _terrainOptions = [[NTTerrainOptions alloc] initWithDataSource:[self demSource]
                                                      elevationDecoder:[self elevationDecoder]];
...
- (void)applyLightOptions {
    if (!_lightOptions) {
        _lightOptions = [[NTLightOptions alloc] init];
...
- (void)applySkyOptions {
    if (!_skyOptions) {
        _skyOptions = [[NTSkyOptions alloc] init];
...
- (void)applyCamera {
    ...
    NSLog(@"CartoDemo: camera lon %.6f lat %.6f zoom %.2f tilt %.1f rotation %.1f",
...
- (enum NTFreeRoamMode)freeRoamMode:(NSString *)name {
...
- (void)startScriptedAnimation {
    ...
    [self after:[DemoConfig floatFor:@"animDelay"] / 1000.0f run:^{
        ...
        if ([anim isEqualToString:@"zoom"]) {
            ...
            NTMapPos *target = [projection fromWgs84:
                [[NTMapPos alloc] initWithX:[DemoConfig doubleFor:@"lon"] + [DemoConfig floatFor:@"animLonDelta"]
...
+ (NTColor *)colorFromHex:(NSString *)hex {
    ...
    return [[NTColor alloc] initWithColor:(int)value];
}
...
- (void)setReliefOutlineEnabled:(BOOL)enabled {
    ...
    NTMapRenderer *renderer = [self.mapView getMapRenderer];
    if (enabled) {
        ...
        _reliefEffect = [[NTPostProcessEffect alloc] initWithName:@"relief_outline"
...
- (void)flyToPeakFinder {
    NTMapPos *focus = [self.mapView getFocusPos];
    ...
    NTMapPos *target = [[NTMapPos alloc] initWithX:[focus getX] y:[focus getY]
...
- (void)applyViewpointElevation {
    NTProjection *projection = [[self.mapView getOptions] getBaseProjection];
    NTMapPos *wgs = [projection toWgs84:[self.mapView getFocusPos]];
    ...
    [self.mapView setFocusPos:[projection fromWgs84:[[NTMapPos alloc] initWithX:[wgs getX]
                                                                              y:[wgs getY]
                                                                              z:meters]]
...
- (void)setArMode:(BOOL)enabled {
    ...
    if (enabled) {
        ...
        [self saveMapAppearance];
        [options setClearColor:[[NTColor alloc] initWithR:0 g:0 b:0 a:0]];
        [options setSkyColor:[[NTColor alloc] initWithR:0 g:0 b:0 a:0]];
...
- (void)setMapLayerOpacity:(float)opacity {
    [_layers enumerateKeysAndObjectsUsingBlock:^(NSNumber *key, NTLayer *layer, BOOL *stop) {
...
- (void)setCameraPreviewEnabled:(BOOL)enabled {
    ...
    if (enabled) {
        ...
        if (!root) {
            NSLog(@"CartoDemo: the map has no superview: no place to put the preview");
            return;
        }
        if (!_cameraPreview) {
 ...
 * 'nutiInterval' ms, which fades the hillshade slot in and out.
 */
- (void)startNutiToggleLoop {
    float interval = [DemoConfig floatFor:@"nutiInterval"] / 1000.0f;
    [self after:interval run:^{
        if (!self->_baseDecoder || ![[DemoConfig stringFor:@"style"] isEqualToString:@"nuti"]) {
            return; // the style was switched away: stop the loop
        }
        self->_nutiParameterOn = !self->_nutiParameterOn;
        [self->_baseDecoder setStyleParameter:[DemoStyles nutiParameter]
                                        value:self->_nutiParameterOn ? @"true" : @"false"];
        [self startNutiToggleLoop];
    }];
}
