...
- (void)start {
    ...
    if (status == AVAuthorizationStatusNotDetermined) {
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
            if (granted) {
                ...
                NSLog(@"MassifDemo: camera permission refused - the AR sky has nothing behind it");
            }
        }];
        return;
    }
    if (status != AVAuthorizationStatusAuthorized) {
        NSLog(@"MassifDemo: no camera permission - grant it in Settings and switch the mode again");
        return;
    }
    ...
    if (!camera) {
        NSLog(@"MassifDemo: no back camera here (expected on the simulator)");
        return;
    }
    NSError *error = nil;
    AVCaptureDeviceInput *input = [AVCaptureDeviceInput deviceInputWithDevice:camera error:&error];
    if (!input) {
        NSLog(@"MassifDemo: could not open the camera: %@", error.localizedDescription);
        return;
    }
    ...
    if (![_session canAddInput:input]) {
        NSLog(@"MassifDemo: the camera input was refused by the session");
        _session = nil;
        return;
    }
