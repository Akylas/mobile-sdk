...
@interface DemoCelestialListener : MSFCelestialEventListener
@end
...
@implementation DemoCelestial {
    ...
    MSFCelestialSprite *_sun;
    MSFCelestialSprite *_moon;
    MSFCelestialArc *_sunPath;
    MSFCelestialArc *_moonPath;
...
- (MSFCelestialLayer *)createLayer:(MSFMapView *)mapView {
    ...
    _sun = [[MSFCelestialSprite alloc] init];
    [_sun setAngularSize:[DemoConfig floatFor:@"celestialSunSize"]];
    [_sun setColor:[[MSFColor alloc] initWithR:255 g:244 b:214 a:255]];
    [_sun setSoftness:0.35f];
    [_sun setClickRadius:3];
    [_sun setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:@"Sun"]];

    _moon = [[MSFCelestialSprite alloc] init];
    [_moon setAngularSize:[DemoConfig floatFor:@"celestialMoonSize"]];
    [_moon setColor:[[MSFColor alloc] initWithR:245 g:245 b:235 a:255]];
    [_moon setSoftness:0.25f];
    [_moon setClickRadius:3];
    [_moon setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:@"Moon"]];

    _sunPath = [[MSFCelestialArc alloc] init];
    [_sunPath setColor:[[MSFColor alloc] initWithR:255 g:216 b:120 a:160]];
    [_sunPath setWidth:[DemoConfig floatFor:@"celestialArcWidth"]];
    [_sunPath setBelowHorizonVisible:NO];
    [_sunPath setClickRadius:2];
    [_sunPath setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:@"Sun path"]];

    _moonPath = [[MSFCelestialArc alloc] init];
    [_moonPath setColor:[[MSFColor alloc] initWithR:170 g:190 b:255 a:130]];
    [_moonPath setWidth:[DemoConfig floatFor:@"celestialArcWidth"]];
    [_moonPath setBelowHorizonVisible:NO];
    [_moonPath setClickRadius:2];
    [_moonPath setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:@"Moon path"]];
...
- (void)update {
    ...
    NSLog(@"MassifDemo: sun az %.0f alt %.0f, moon az %.0f alt %.0f, hour %.2f UTC %d-%d-%d",
...
- (MSFDoubleVector *)dailyPath:(BOOL)isSun lat:(double)lat lon:(double)lon {
...
- (void)updateMoonPhase:(double)n {
    ...
    [_moon setBitmap:[MSFBitmapUtils createBitmapFromUIImage:image]];
}

- (MSFCelestialSprite *)addAircraft:(double)lon lat:(double)lat altitude:(double)altitudeMeters {
    MSFCelestialSprite *aircraft = [[MSFCelestialSprite alloc] init];
    [aircraft setScreenSize:24];
    [aircraft setColor:[[MSFColor alloc] initWithR:255 g:255 b:255 a:255]];
    [aircraft setPosition:[[MSFMapPos alloc] initWithX:lon y:lat] altitude:altitudeMeters];
    [aircraft setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:@"Aircraft"]];
    [_layer add:aircraft];
    return aircraft;
}
