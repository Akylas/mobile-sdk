...
#import "MSFCelestialObject.h"
#import "MSFCelestialSprite.h"
#import "MSFCelestialArc.h"
#import "MSFCelestialEventListener.h"
...
@interface DemoStarsListener : MSFCelestialEventListener
@end

@implementation DemoStarsListener

- (BOOL)onCelestialObjectClicked:(MSFClickInfo *)clickInfo celestialObject:(MSFCelestialObject *)object {
    ...
    if ([object isKindOfClass:[MSFCelestialSprite class]]) {
...
@implementation DemoStars {
    MSFCelestialLayer *_layer;
    NSMutableArray<MSFCelestialSprite *> *_stars;
    NSMutableArray<NSValue *> *_starEquatorial;         // DemoEquatorial per star
    NSMutableArray<MSFCelestialArc *> *_figures;
    NSMutableArray<MSFCelestialSprite *> *_figureLabels;
    NSMutableArray<NSArray<NSValue *> *> *_figureEquatorial; // per figure, pairs of DemoEquatorial
    NSMutableArray<MSFCelestialSprite *> *_planets;
    MSFCelestialArc *_equator;
    DemoStarsListener *_listener;
    CGFloat _scale;
}

- (MSFCelestialLayer *)createLayer:(MSFMapView *)mapView {
    _layer = [[MSFCelestialLayer alloc] init];
    ...
    for (NSString *entry in [DemoStarCatalogue stars]) {
        ...
        if (parts.count != 4) {
            NSLog(@"MassifDemo: bad catalogue entry: %@", entry);
            continue;
        }
        ...
        MSFCelestialSprite *star = [[MSFCelestialSprite alloc] init];
        ...
        [star setColor:[[MSFColor alloc] initWithR:255 g:255 b:250 a:255]];
        [star setSoftness:0.45f];
        [star setClickRadius:1.5f];
        [star setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:name]];
        [star setMetaDataElement:META_INFO
                         element:[[MSFVariant alloc] initWithString:[NSString stringWithFormat:@"mag %@", parts[3]]]];
        [_layer add:star];
    ...
    for (NSString *figureName in [DemoStarCatalogue figureNames]) {
        ...
        for (NSUInteger i = 0; i + 1 < names.count; i += 2) {
            ...
            if (!from || !to) {
                NSLog(@"MassifDemo: %@: no such star %@", figureName, from ? names[i + 1] : names[i]);
                continue;
            }
        ...
        MSFCelestialArc *arc = [[MSFCelestialArc alloc] init];
        [arc setColor:[[MSFColor alloc] initWithR:120 g:170 b:255 a:110]];
        ...
        [arc setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:figureName]];
        [arc setMetaDataElement:META_INFO
                        element:[[MSFVariant alloc] initWithString:
        ...
        MSFCelestialSprite *label = [[MSFCelestialSprite alloc] init];
        MSFBitmap *labelBitmap = [self textBitmap:figureName];
        ...
        [label setColor:[[MSFColor alloc] initWithR:255 g:255 b:255
                                                 a:(int)roundf(255 * [DemoConfig floatFor:@"starsLabelOpacity"])]];
        [label setClickRadius:0]; // the figure itself is the clickable thing
        [label setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:figureName]];
        [label setMetaDataElement:META_INFO element:[[MSFVariant alloc] initWithString:@""]];
        [_layer add:label];
        [_figureLabels addObject:label];
    }

    for (NSString *name in [DemoAstro planetNames]) {
        MSFCelestialSprite *planet = [[MSFCelestialSprite alloc] init];
        ...
        [planet setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:name]];
        [planet setMetaDataElement:META_INFO element:[[MSFVariant alloc] initWithString:@"planet"]];
        [_layer add:planet];
    ...
    _equator = [[MSFCelestialArc alloc] init];
    [_equator setColor:[[MSFColor alloc] initWithR:90 g:200 b:190 a:90]];
    [_equator setWidth:1.5f];
    [_equator setBelowHorizonVisible:NO];
    [_equator setClickRadius:1.5f];
    [_equator setMetaDataElement:META_NAME element:[[MSFVariant alloc] initWithString:@"Celestial equator"]];
    [_equator setMetaDataElement:META_INFO element:[[MSFVariant alloc] initWithString:@"declination 0"]];
    [_layer add:_equator];
...
- (void)updateWithN:(double)n lat:(double)lat lon:(double)lon {
    ...
    for (NSUInteger i = 0; i < _stars.count; i++) {
        ...
        MSFCelestialSprite *star = _stars[i];
    ...
    for (NSUInteger i = 0; i < _figures.count; i++) {
        ...
        MSFDoubleVector *directions = [[MSFDoubleVector alloc] init];
        ...
        MSFCelestialSprite *label = _figureLabels[i];
 ...
 * texture.
 */
- (MSFBitmap *)textBitmap:(NSString *)text {
    ...
    return [MSFBitmapUtils createBitmapFromUIImage:image];
}
...
- (MSFColor *)planetColor:(NSString *)name {
    if ([name isEqualToString:@"Mars"]) {
        return [[MSFColor alloc] initWithR:255 g:130 b:90 a:255];
    }
    if ([name isEqualToString:@"Venus"]) {
        return [[MSFColor alloc] initWithR:255 g:250 b:220 a:255];
    }
    if ([name isEqualToString:@"Jupiter"]) {
        return [[MSFColor alloc] initWithR:255 g:235 b:180 a:255];
    }
    if ([name isEqualToString:@"Saturn"]) {
        return [[MSFColor alloc] initWithR:240 g:220 b:160 a:255];
    }
    return [[MSFColor alloc] initWithR:220 g:220 b:230 a:255];
}
