...
ENUM_TEMPLATE = """
%typemap(jstype) $ENUMNAME$ "@$JAVAENUM$.Value int"
%typemap(jstype) const $ENUMNAME$& "@$JAVAENUM$.Value int"
"""

# Java classes generated by enumtypeunsafe.swg, to be given their @IntDef after Swig runs.
javaEnums = []
...
def addIntDefAnnotation(fileName):
  # Swig knows the enum's type but not its members, so the @IntDef is written here, where the
  # generated constants can just be read back out of the file.
  if not os.path.isfile(fileName):
    return

  lines_in = readLines(fileName)
  constants = [match.group(1) for match in
               filter(None, (re.match(r'\s*public final static (?:@\S+\s+)?int (\w+)\s*=', line) for line in lines_in))]
  if not constants:
    return
  ...
  # Declared last so the constants it lists are already in scope.
  closing = max(index for index, line in enumerate(lines_out) if line.rstrip() == '}')
  lines_out[closing:closing] = [
    '\n',
    '  /**\n',
...
ignoredSourceFiles = []
argsDefines = []
def transformSwigFile(sourcePath, outPath, headerDirs, packageName):
  ...
  for line in lines_in:
    ...
    if match:
      ...
      javaClass = enumName.split("::")[-1]
      javaEnum = "com.massifmaps.%s.%s" % (packageName, javaClass)
      args = { 'ENUMNAME': enumName, 'JAVAENUM': javaEnum }
      lines_out += applyTemplate(ENUM_TEMPLATE, args)
      javaEnums.append((packageName, javaClass))
      continue
...
def transformSwigPackage(args, sourceDir, outDir, packageName):
  for fileName in os.listdir(sourceDir):
    ...
    transformSwigFile(sourcePath, outPath, headerDirs, packageName)
  return True
...
def runSwigJobs(jobs):
  ...
  for packageName, className in javaEnums:
    proxyDir = os.path.join(args.proxyDir, ("com.massifmaps.%s" % packageName).replace(".", "/"))
    addIntDefAnnotation(os.path.join(proxyDir, className + ".java"))
  return True
