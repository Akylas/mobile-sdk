...
def buildSwigPackage(args, sourceDir, packageName):
  for fileName in os.listdir(sourceDir):
    ...
    cmd = [args.swig, "-c++", "-doxygen", "-objc", "-outdir", proxyDir, "-o", outPath] + defines + includes + [sourcePath]
...
parser.add_argument('--cppdir', dest='cppDir', default='../all/native;../ios/native', help='directories containing C++ headers')
...
if not checkExecutable(args.swig, '-help'):
  print('Unable to find SWIG executable. Use --swig argument to specify its location. The supported version is available from https://github.com//massif-maps/mobile-swig')
  sys.exit(-1)
